...
export const FAQS = [
  ...
  { title:'有风险说明但没有掩码，算检测失败吗？', body:'语义判断与空间定位是不同步骤。模型可能发现风险，但无法明确定位缺失的护栏、安全带等对象。请保留判断和疑问，由安全人员核对原图，不能把“无掩码”当作“无风险”。', target:'/detection-results', action:'查看风险与证据' },
  ...
  { title:'为什么看到预先编辑的案例和数据？', body:'这是随包保留的展示素材，界面会标记其来源。“展示视图”控制外观；“预置展示素材”控制数据展示，两者互不等同。关闭素材显示不会删除文件或真实历史任务。', target:'/system-settings?tab=workspace', action:'设置展示方式' },
  { title:'换电脑要带哪些内容？', body:'完整复制交付文件夹，不要只拿走 EXE。演示带便携 Python 与构建好的界面；系统还需 WebView2。真实检测需另外安装完整推理环境、下载权重并更新路径。业务备份只包含业务库，完整迁移还需输出图片、规范、配置与运行数据。', target:'/system-settings?tab=about', action:'查看部署说明' },
