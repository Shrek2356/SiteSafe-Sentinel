<template>
  <section class="deployment-guide">
    ...
    <template v-if="mode === 'demo'">
      ...
      <a-space wrap class="guide-actions"><router-link to="/detection-results"><a-button type="primary">查看八个展示案例</a-button></router-link><a v-if="safeSource(links.webview2)" :href="safeSource(links.webview2)" target="_blank" rel="noopener noreferrer">WebView2 官方说明</a></a-space>
    </template>
    <template v-else>
      ...
      <article class="step-panel" v-if="step === 0">
        ...
        <p>{{ mode === 'offline' ? '本地 8B 检测建议 32 GB 内存、16 GB 以上 NVIDIA 显存；12 GB 仅属于需实测的尝试条件。' : '云端只替代视觉大模型，本机仍需能运行 YOLO / SAM3 的 NVIDIA GPU 和 CUDA PyTorch。' }} 多模型同时运行可能显存不足，检查通过不保证速度或并发能力。</p>
      ...
      <article class="step-panel" v-else-if="step === 1">
        <h3>02 · 取得模型与配套组件</h3><p>建议放在下列包内目录。项目 YOLO 下载地址暂留空，后续填入 GitHub 地址即可；没有链接不会被标为“已下载”。</p>
      ...
      <article class="step-panel" v-else-if="step === 3">
        ...
        <p v-if="mode === 'offline'">检查缺项修复后，在模型运行时页激活所需组件，点击“启动 Qwen”，等待“测试连接”成功。SAM3、YOLO 与可选 CLIP 在任务到来时加载，不需要单独启动服务。</p>
      ...
      <article class="step-panel" v-else>
        ...
        <ol><li>在实时检测明确选择{{ mode === 'offline' ? '本地离线' : '云端' }}，提交一张新图片，确认产生新的任务记录。</li><li>核对风险描述、证据、疑问说明及可用的框/掩码。无掩码不等于没有发现风险。</li><li>完成八个示例的真实回归，记录模型、环境和结果；不把已有展示报告当作新设备推理结果。</li><li>需要实时视频时，再验证 YOLO 初筛与周期全面检测；缺少 YOLO 权重不能称实时链路完整。</li></ol>
