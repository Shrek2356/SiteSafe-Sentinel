      ...
      <a-tab-pane key="kb" tab="安全知识库管理">
        ...
        <a-space wrap style="margin-bottom: 14px">
          ...
          <a-input-search
            ...
            placeholder="输入问题验证规范检索，如：高处作业如何防坠落"
            style="width: 420px"
            enter-button="检索验证"
        ...
        <a-spin :spinning="runtimeLoading">
          <a-form layout="vertical" class="runtime-form">
            <a-card size="small" class="service-card" title="模型组件激活控制台">
              ...
              <p class="tip">YOLO、SAM3、CLIP 采用任务级懒加载；激活不会立即占用显存，下一个检测任务才加载。Qwen 需在下方单独启动推理服务。</p>
            </a-card>
    ...
    <a-modal v-model:open="pushEditorOpen" title="编辑告警推送规则" @ok="savePushRule">
      <a-form layout="vertical">
        ...
        <a-form-item label="推送对象">
          <a-select v-model:value="pushEditor.channels" mode="tags" placeholder="输入对象后回车，如 安全员">
            <a-select-option value="安全员">安全员</a-select-option>
...
const runtimeForm = ref({ qwen_enabled: true, yolo_enabled: true, sam3_enabled: true, clip_enabled: false })
...
const runtimeFields = [
  ...
  { key: 'yolo_css_weights_path', label: 'YOLO CSS 权重', placeholder: 'CSS best.pt 文件路径' },
  { key: 'yolo_construction_weights_path', label: 'YOLO ConstructionSite 权重', placeholder: 'ConstructionSite best.pt 文件路径' },
]
const modelComponents = [
  { key: 'qwen_enabled', validation: 'qwen_runtime', name: 'Qwen3-VL', description: '异常语义识别与结构化 RiskSpec' },
  { key: 'yolo_enabled', validation: 'yolo_runtime', name: 'YOLO26', description: '实时视觉注意力初筛与候选区域' },
