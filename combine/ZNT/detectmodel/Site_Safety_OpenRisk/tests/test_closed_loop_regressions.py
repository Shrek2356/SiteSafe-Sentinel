...
def test_mobile_manual_report_enters_event_order_and_persists_image(tmp_path) -> None:
    ...
    with client:
        response = client.post(
            "/api/mobile/report",
            data={"title": "临边无防护", "level": "red", "area": "二层平台", "desc": "现场确认"},
            files={"files": ("evidence.jpg", content.getvalue(), "image/jpeg")},
            headers=headers,
        )
    assert response.status_code == 200
    assert response.json()["workOrders"] == 1
...
def test_handled_alarm_is_removed_from_pending_alarm_list(tmp_path) -> None:
    ...
    event = {
        ...
        "risks": [{
            "risk_id": "missing_helmet", "risk_name_zh": "未戴安全帽", "verified": True,
...
def test_concurrent_duplicate_ingest_creates_one_order(tmp_path) -> None:
    ...
    event = {
        ...
        "media": {"image_path": "", "image_width": 0, "image_height": 0},
        "overall_has_anomaly": True,
        "risks": [{"risk_id": "missing_helmet", "risk_name_zh": "未戴安全帽",
...
def test_model_config_includes_all_operator_defaults_and_approved_overrides(monkeypatch, tmp_path) -> None:
    ...
    override_path.write_text(
        json.dumps({"risk_overrides": {"missing_helmet": {"min_verified_confidence": 0.85}}}),
        encoding="utf-8",
    )
    ...
    assert len(rows) >= 11
    assert rows["missing_helmet"]["approved"] is True
    assert rows["missing_helmet"]["source"] == "approved_override"
    assert rows["missing_edge_protection"]["approved"] is False
    assert rows["missing_edge_protection"]["value"] == 0.60
    db.conn.close()
