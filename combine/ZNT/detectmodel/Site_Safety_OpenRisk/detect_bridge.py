...
class DetectBridge:
    def __init__(self, default_profile: str = "") -> None:
        ...
        self.jobs_root = ROOT / "outputs" / "bridge_jobs"
        ...
        self.knowledge_base = KnowledgeBase(ROOT / "knowledge_base")
    ...
    def health(self) -> dict:
        ...
        return {
            ...
            "code_root": str(ROOT),
            "time": _now(),
            "hint": "standard=云端Qwen+本地SAM3；offline=本地Qwen+YOLO+SAM3；demo=演示",
        ...
        try:
            ...
            if job.get("profile") != "demo":
                ...
                config.setdefault("pipeline", {})["threshold_overrides_path"] = (
                    "configs/threshold_overrides.json"
                )
    ...
    def _build_event(self, job: dict, out_dir: Path) -> Optional[dict]:
        try:
            ...
            reasoning = RiskReasoningAgent(ROOT / "examples" / "regulations.json")
            device = DeviceInfo(
            ...
            for risk in payload.get("risks", []):
                query_parts = [
                    risk.get("risk_name_zh", ""),
                    risk.get("risk_description", ""),
                    " ".join(risk.get("visible_evidence") or []),
                ]
    ...
    def _push_business_event(
        ...
        business_api = os.getenv("ZNT_BUSINESS_API", "http://127.0.0.1:8800").rstrip("/")
    ...
    def _build_frontend_result(
        ...
        for i, r in enumerate(risks):
            if not r.get("overlay") and i < len(overlays):
                r["overlay"] = self._media_url(job["job_id"], overlays[i].name)
            if not r.get("mask") and i < len(masks):
                r["mask"] = self._media_url(job["job_id"], masks[i].name)
        ...
        return {
            "overall_has_anomaly": bool(risks),
