...
offline   离线：本地 Qwen + YOLO + SAM3（CLIP 可选）
demo      演示：Mock
"""
...
PROFILES: Dict[str, Dict[str, Any]] = {
    "standard": {
        ...
        "config": "configs/qwen_visual.yaml",
    ...
    "offline": {
        ...
        "name": "离线",
        "recommend": True,
        "summary": "YOLO初筛 + 本地Qwen + SAM3",
        "stack": "YOLO26 + 本地Qwen3-VL + SAM3",
        "config": "configs/qwen_local_production.yaml",
        "needs": ["local_qwen_config", "sam3_weights", "yolo_weights"],
    },
    "demo": {
        "id": "demo",
        "name": "演示",
...
def probe_capabilities(root: Path = ROOT) -> Dict[str, bool]:
    ...
    combine_root = root.parent.parent.parent
    yolo_dir = combine_root / "yolo_site_workspace_portable" / "weights"
    yolo = bool(runtime["yolo_enabled"]) and ((yolo_dir / "yolo26m_css_v28_baseline_best.pt").is_file() and (
        yolo_dir / "yolo26m_construction_site_best.pt"
    ).is_file() or (
        checks["yolo_css_weights_path"]["ok"]
        and checks["yolo_construction_weights_path"]["ok"]
    ))
