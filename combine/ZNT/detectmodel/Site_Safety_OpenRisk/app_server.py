...
UI_DIR = ROOT / "demo_ui"
THRESHOLD_OVERRIDES_PATH = ROOT / "configs" / "threshold_overrides.json"
ROLE_RANK = {"viewer": 0, "safety_officer": 1, "admin": 2}
TOKEN_TTL_HOURS = 12
INTERNAL_KEY = "local-detect-bridge"
PUSH_RULES_PATH = ROOT / "configs" / "notify_targets.json"
...
class AppState:
    def __init__(self, db: Database, app_data: Path) -> None:
        ...
        self.reasoning = RiskReasoningAgent(ROOT / "examples" / "regulations.json")
        ...
        self.response = CollaborativeResponseAgent(gateway=None)
        if PUSH_RULES_PATH.is_file():
            try:
                ...
                pass
        self.learning = ReviewLearningAgent(app_data / "case_library.jsonl")
        self.kb = KnowledgeBase(ROOT / "knowledge_base")
        ...
        self.detect_api = os.environ.get("ZNT_DETECT_API", "http://127.0.0.1:8810").rstrip("/")
    ...
    def _decide_confirmation(self, request_id: str, payload: dict, by: str) -> dict:
        ...
        if verdict == "confirmed":
            ...
            new_order = WorkOrder(
                ...
                status="confirmed",
                created_at=now,
                due_at=(datetime.now().astimezone() + timedelta(hours=SLA_HOURS[level])).isoformat(timespec="seconds"),
    ...
    def start_stream(self, source_id: str) -> dict:
        ...
        log_dir = ROOT / "app_data" / "stream_logs"; log_dir.mkdir(parents=True, exist_ok=True)
    ...
    def stream_log(self, source_id: str, lines: int = 120) -> dict:
        if self.db.get(source_id) is None:
            raise HTTPException(404, "视频源不存在")
        path = ROOT / "app_data" / "stream_logs" / f"{source_id}.log"
...
app = FastAPI(title="工地安全智能体系统", version="1.0", lifespan=lifespan)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def login(payload: dict) -> dict:
    ...
    return {"token": result["token"], "user": {"id": f"u-{username}", "username": username,
            "name": {"admin": "系统管理员", "safety": "安全员", "viewer": "项目总监"}.get(username, username),
            "role": front_role}}
...
def frontend_projects(user: dict = Depends(auth_viewer)) -> List[dict]:
    sites = STATE.sites()
    if not sites:
        return [{"id": "SITE-DEFAULT", "name": "智慧城建·滨江一期工地", "shortName": "滨江一期", "address": "本地演示项目"}]
...
def frontend_dashboard(projectId: str = "", user: dict = Depends(auth_viewer)) -> dict:
    ...
    return {"projectId": projectId or "SITE-DEFAULT", "projectName": projectId or "智慧城建·滨江一期工地",
...
def mobile_report(
    ...
    event = DetectionEvent(
        ...
        time=TimeInfo(captured_at=now, detected_at=now, reported_at=now),
        media=MediaInfo(image_path=saved_images[0] if saved_images else "",
                        image_width=image_width, image_height=image_height),
        scene_summary=f"安全员人工上报：{title}", overall_has_anomaly=True,
        risks=[RiskFinding(
            risk_id=risk_id, risk_name_zh=title, verified=True, confidence=confidence,
            risk_level="general", risk_level_zh="一般风险",
            geometry=GeometryInfo(), visible_evidence=[desc.strip() or "安全员现场确认"],
            risk_description=desc.strip() or f"{area}发现{title}",
            manual_review_required=False,
        )],
        review=ReviewInfo(status="confirmed", reviewer=user["username"],
                          reviewed_at=now, comment="安全员移动端人工上报"),
    )
    try:
...
def frontend_model_config(user: dict = Depends(auth_viewer)) -> dict:
    ...
    operator_names = {
        ...
        "worker_under_suspended_load": "人员位于悬吊物下方",
        "fallen_worker": "人员异常倒地",
        "blocked_passage": "通道被材料堵塞",
        "smoke_or_fire": "烟雾或火情",
        "missing_edge_protection": "临边或洞口防护缺失",
        "missing_fall_protection": "高处作业防坠保护缺失",
        "machinery_proximity": "人员与机械距离过近",
        "collapsed_scaffold": "脚手架变形或坍塌",
        "electrical_cable_on_wet_floor": "电缆接触潮湿地面",
        "unsafe_scaffold_platform": "脚手架平台防护不完整",
    }
    overrides = STATE.load_overrides().get("risk_overrides", {})
    all_keys = list(operator_names)
    all_keys.extend(key for key in overrides if key not in operator_names)
    thresholds = []
    for key in all_keys:
...
def main() -> None:
    ...
    parser.add_argument("--db", default="app_data/site_safety.db")
