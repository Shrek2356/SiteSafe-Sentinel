...
UNITS = ('app_data', 'knowledge_base', 'outputs/bridge_jobs',
         'outputs/runtime/runtime_settings.json', 'configs/runtime_initial_settings.json',
         'configs/threshold_overrides.json', 'configs/notify_targets.json')
...
def inspect_archive(path: Path, destination: Path | None = None) -> dict:
    with zipfile.ZipFile(path) as archive:
        ...
        if manifest.get('format') != FORMAT or manifest.get('units') != list(UNITS): raise ValueError('不是兼容的 SiteSafe 工作空间备份')
...
def prepare_data(stage: Path, old_root: str, new_root: str):
    ...
    db = stage / 'app_data/site_safety.db'
    if db.is_file():
        with closing(sqlite3.connect(db)) as conn:
            ...
            conn.commit()
    for relative in ['outputs/runtime/runtime_settings.json', 'configs/runtime_initial_settings.json']:
        path = stage / relative
        data = json.loads(path.read_text(encoding='utf-8')) if path.is_file() else {}
        data['qwen_autostart'] = False
        _json(path, data)
    for path in stage.glob('outputs/bridge_jobs/JOB-*/bridge_summary.json'):
