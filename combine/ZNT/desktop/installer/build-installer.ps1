...
$exeVersion = [Diagnostics.FileVersionInfo]::GetVersionInfo((Join-Path $payloadInfo.payload 'SiteSafe-Sentinel.exe')).ProductVersion
if ($exeVersion -ne $version) { throw "载荷中的 EXE/前端版本不一致：$exeVersion / $version" }
$outputExe = Join-Path $outputRoot "SiteSafe-Sentinel_Setup_v${version}_x64.exe"
...
$receipt = [ordered]@{
    product='SiteSafe-Sentinel'; version=$version; built_at=(Get-Date).ToUniversalTime().ToString('o')
    ...
    app_id='{D8200384-F185-493C-B501-66A01C93F267}'; install_scope='current Windows user'
