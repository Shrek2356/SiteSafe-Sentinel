...
def workspace(root, label='original'):
    ...
    for directory in ['app_data','knowledge_base','outputs/bridge_jobs/JOB-1','outputs/runtime','configs']:
        (backend/directory).mkdir(parents=True,exist_ok=True)
    image=backend/'outputs/bridge_jobs/JOB-1/overlay.png';image.write_bytes(label.encode())
    with sqlite3.connect(backend/'app_data/site_safety.db') as conn:
...
def test_offline_roundtrip_preserves_evidence_rebases_paths_and_pauses_models(tmp_path):
    ...
    assert 'knowledge_base/rules.txt' in names and 'app_data/site_safety.db' in names
    ...
    assert (backend/'outputs/bridge_jobs/JOB-1/overlay.png').read_bytes()==b'existing'
    result=maintenance.run(lambda:None)
    assert result['status']=='restored', result
    assert Path(result['recovery_backup']).is_file()
    assert (backend/'outputs/bridge_jobs/JOB-1/overlay.png').read_bytes()==b'original'
    with sqlite3.connect(backend/'app_data/site_safety.db') as conn:
...
@pytest.mark.parametrize('malicious',['../escape.txt','app_data/../../escape.txt','app_data/C:evil','app_data/folder./evil.txt'])
...
def test_restore_failure_and_crash_journal_roll_back(tmp_path,monkeypatch,interrupted):
    ...
    def fail_once(path,dest):
        nonlocal failed
        if not failed and 'incoming' in path.parts and path.name=='knowledge_base':
            failed=True
