...
| 使用方式 | 基础环境 | 是否需要模型 |
|----------|----------|--------------|
| v1.3 完整桌面包，仅演示 | 随包 Python、业务依赖与前端构建；无需系统 Node | 不需要 |
...
3. 要检测新图片，再运行 `部署助手.bat`，按照实际显卡和模型准备完整环境。
...
- Python、PyTorch、torchvision 与 CUDA 按检测环境整组匹配；不要把 Demo 的 Python 3.12 快照当作历史 GPU 环境的直接替代。
- `desktop-settings.json` 的 `backend_python` 是桌面后台选择；兼容批处理的环境查找顺序不是桌面 EXE 的默认行为。
